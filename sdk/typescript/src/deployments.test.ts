...
import { deploymentDefaults, renderDeploymentModule } from "./deployments.js";

const RECORDS_URL = new URL("../../../deploy/deployments.json", import.meta.url);

test("the committed deployments.json yields a usable studio-next default", () => {
  const records = JSON.parse(readFileSync(RECORDS_URL, "utf-8"));
  const d = deploymentDefaults(records, "studio-next");
  assert.equal(d.network, "studio-next");
  ...
  if (d.seeded) {
    assert.match(d.seeded.address, /^0x[0-9a-fA-F]{40}$/);
    assert.equal(d.testMonocle, d.seeded.address);
  }
});
...
test("rendered module is valid TypeScript-looking source containing the factory", () => {
  const records = JSON.parse(readFileSync(RECORDS_URL, "utf-8"));
  const d = deploymentDefaults(records, "studio-next");
  const src = renderDeploymentModule(d);
  assert.match(src, /export const DEPLOYMENT: DeploymentDefaults = \{/);
  assert.ok(src.includes(d.factory));
});

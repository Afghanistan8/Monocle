...
The GitHub Actions workflow (`.github/workflows/ci.yml`) defines the same checks. It is currently
manual-only (`workflow_dispatch`); restore the `push` / `pull_request` triggers there to run it on
every push.

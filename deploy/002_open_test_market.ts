 ...
 * (testMonocle, createTx, submitTxs, adjudicateTx, roundStatus, liveUrl) and
 * regenerates frontend/lib/deployment.ts so the site lists it by default.
...
const SITE = process.env.MONOCLE_SITE_URL ?? "https://monocle-ten.vercel.app";
...
function saveRecord(network: string, patch: Record<string, unknown>) {
  ...
  writeFrontendDeployment(network);
}
...
async function finishLifecycle(m: MonocleCalls, networkName: string) {
  ...
  saveRecord(networkName, { roundStatus: round.status, ...(claimTx ? { finalizeClaimTx: claimTx } : {}) });
...
async function main() {
  ...
  if (finishOnly) {
    ...
    step(`Done. Live page: ${SITE}/m/${recorded}`);
    return;
  }
  ...
  step(`Done. Live page: ${SITE}/m/${monocle}`);
}
